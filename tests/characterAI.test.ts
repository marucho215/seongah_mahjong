...
describe("causality trace: each special-discard-mechanic character populates only its own fields", () => {
  ...
  it("Optima: optimaScoreGap is null (not a misleading 0) whenever the pool has exactly 1 candidate", () => {
    // Search across seeds/hands for a real single-candidate-pool case, the same
    // "search across many seeds" pattern used elsewhere in this codebase for scenarios
    // that aren't reliably producible from one fixed hand.
    let found = 0;
