  ...
  it("does NOT end the game when a noten payment lands a player at exactly 0 (only score < 0 counts)", () => {
    ...
    for (const seed of [4, 23, 28]) {
      ...
      for (const { start, end } of handSlices(log)) {
        ...
        // either another hand_start (game continues) or a non-tobi game_end ("length" or
        // "extension_end" - Phase 2's target-score extension can legitimately still be
        // running with LOW_START_RULES's tiny starting score) - never "tobi"
        const next = log[end];
        expect(next).toBeDefined();
        if (next!.type === "game_end") expect(next!.reason).not.toBe("tobi");
