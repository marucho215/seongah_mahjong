...
import { isHonor, isTerminalOrHonor, parseKind } from "../core/tiles.js";
...
export class CharacterAI {
  ...
  chooseDiscard(hand: Hand, ctx: CharacterDecisionContext): number {
    ...
    // Pass 2 (below) only spends the expensive full ukeire computation on candidates that
    // are actually shanten-tied for best, since SHANTEN_WEIGHT dominates the score anyway -
    // a candidate that's strictly worse on shanten can never be selected regardless of ukeire.
    ...
    for (const kind of uniqueKinds) {
      ...
      const needsExactUkeire = this.useExactUkeireForAllCandidates || shanten <= minCandidateShanten;
      const ukeire = needsExactUkeire ? computeWinningTiles(remainingCounts, existingMelds, ctx.rules).length : 0;
    ...
    const speedWeight = clamp(0.25 + (p.aggression - 0.5) * 0.3 - (p.valueGreed - 0.5) * 0.15, 0.05, 0.6);
    ...
    for (const c of candidates) {
      ...
      if (p.experimentBias !== undefined) {
        ...
        score = this.applyExperimentBias(score, c, baseCounts, existingMelds, p);
  ...
  private applyExperimentBias(
    ...
    // Route selection: once a route looks plausible, keep favoring it (routePersistence)
    // until it becomes impossible, badly behind standard efficiency, or the wall runs low.
    if (this.experimentRoute === null && existingMelds === 0) {
      const pairs = baseCounts.filter((n) => n >= 2).length;
      if (pairs >= 3 && c.chiitoiFit > 0) this.experimentRoute = "chiitoi";
      else if (c.honitsuFit > 0.55) this.experimentRoute = "honitsu";
    }
    if (this.experimentRoute === "chiitoi") {
      if (c.chiitoiFit === 0 || c.shanten - Math.min(c.shanten, 8) > 3) {
        this.routeCommittedTurns++;
        if (this.routeCommittedTurns > 2) this.experimentRoute = null;
      } else {
        score += computeExperimentBonus(true, c.chiitoiFit, p.experimentBias ?? 0, p.routePersistence ?? 0);
      }
    } else if (this.experimentRoute === "honitsu") {
      if (c.honitsuFit === 0) {
        this.routeCommittedTurns++;
        if (this.routeCommittedTurns > 2) this.experimentRoute = null;
      } else {
  ...
  private isDamaViable(hand: Hand, tileIdToDiscard: number, ctx: CharacterDecisionContext): boolean {
    ...
    for (const kind of winningKinds.slice(0, 3)) {
  ...
  shouldDeclareShouminkan(): boolean {
    return true; // upgrading one's own pon is essentially free; not a discretionary "call" in the personality sense
  }

  shouldDeclareAnkan(): boolean {
    return true; // a closed kan never opens the hand; not personality-discretionary
  }
  ...
  private evaluateCallAdvantage(
    ...
    const beforeUkeire = computeWinningTiles(beforeCounts, hand.melds.length, ctx.rules).length;
    ...
    const afterUkeire = computeWinningTiles(afterCounts, meldsAfter, ctx.rules).length;
