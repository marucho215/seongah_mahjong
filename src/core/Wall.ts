...
export class Wall {
  ...
  private kansDrawn = 0;
  private kanRinshanDrawn = 0;
  private pendingKanReplacementDraws = 0;
  /** Extra kan-dora pairs currently visible. Kept separate from committed kans because
   *  Mahjong Soul reveals daiminkan/shouminkan dora only after the caller's discard. */
  private revealedKanDoraCount = 0;
  /** Committed open/added kans whose indicator pair has not been revealed yet. */
  private pendingKanDoraCount = 0;
   ...
   *  rule-capped) plus rules.northTileCopies (kita is physically capped by the tile set's North tiles
  ...
  private readonly initialReplacementSlots: number;
  private readonly doraIndicatorSlots: number;
  private readonly wallLayout: RuleConfig["wallLayout"];
  ...
  constructor(rules: RuleConfig, seed: Seed) {
    ...
    this.deadWall = this.allTiles.slice(this.allTiles.length - rules.deadWallSize);
    this.liveEnd = this.allTiles.length - rules.deadWallSize;
    this.replacementDrawCapacity = rules.maxKans + (rules.kitaEnabled ? rules.northTileCopies : 0);
    this.initialReplacementSlots = rules.initialReplacementSlots;
    this.doraIndicatorSlots = rules.doraIndicatorSlots;
    this.wallLayout = rules.wallLayout;
    // Sanma has R1..R8 followed by three initial indicator pairs. Later pairs come from
    // absorbed tiles. Yonma has R1..R4 followed by all five standard indicator pairs.
    const indicatorStart = rules.initialReplacementSlots;
    const initialIndicatorPairs = rules.wallLayout === "yonma-standard" ? rules.doraIndicatorSlots : 3;
    this.initialDoraSlots = Array.from({ length: initialIndicatorPairs }, (_, i) => this.deadWall[indicatorStart + i * 2]!);
    this.initialUraSlots = Array.from({ length: initialIndicatorPairs }, (_, i) => this.deadWall[indicatorStart + i * 2 + 1]!);
  }
  ...
  private rinshanPoolUsed(): number {
    return this.kanRinshanDrawn + this.kitaRinshanDrawn;
  }
  ...
  canDrawRinshan(maxKans: number): boolean {
    return this.kansDrawn < maxKans && this.pendingKanReplacementDraws === 0 && this.canTakeReplacementTile();
  }

  /** Commits a kan independently from consuming its replacement tile. A later kan first
   *  reveals any indicator deferred by an earlier open/added kan. */
  commitKan(revealTiming: "immediate" | "after-discard"): void {
    if (this.pendingKanReplacementDraws > 0 || !this.canTakeReplacementTile()) {
      throw new Error("Wall: cannot commit another kan");
    }
    this.revealPendingKanDora();
    this.kansDrawn++;
    this.pendingKanReplacementDraws++;
    if (revealTiming === "immediate") this.revealedKanDoraCount++;
    else this.pendingKanDoraCount++;
  }

  /** Draws the physical replacement belonging to the already committed kan. */
  drawCommittedRinshan(): Tile {
    if (this.pendingKanReplacementDraws <= 0) {
      throw new Error("Wall: no committed kan replacement is pending");
    }
    const tile = this.takeReplacementTile();
    this.pendingKanReplacementDraws--;
    this.kanRinshanDrawn++;
    return tile;
  }

  /** Backward-compatible immediate-ankan convenience used by direct Wall callers. */
  drawRinshan(revealTiming: "immediate" | "after-discard" = "immediate"): Tile {
    this.commitKan(revealTiming);
    return this.drawCommittedRinshan();
  }

  /** Reveals every indicator deferred by a completed open/added kan. Idempotent. */
  revealPendingKanDora(): void {
    this.revealedKanDoraCount += this.pendingKanDoraCount;
    this.pendingKanDoraCount = 0;
  }

  pendingKanDora(): number {
    return this.pendingKanDoraCount;
  }
  ...
  private takeReplacementTile(): Tile {
    ...
    let tile: Tile;
    if (this.nextRinshanSlot < this.initialReplacementSlots) {
   ...
   *  are sourced from tiles absorbed during the corresponding replacement draws. */
  doraIndicators(): Tile[] {
    const count = Math.min(1 + this.revealedKanDoraCount, this.doraIndicatorSlots);
    if (this.wallLayout === "yonma-standard") return this.initialDoraSlots.slice(0, count);
  ...
  uraDoraIndicators(): Tile[] {
    const count = Math.min(1 + this.revealedKanDoraCount, this.doraIndicatorSlots);
    if (this.wallLayout === "yonma-standard") return this.initialUraSlots.slice(0, count);
